...
mod BaseDAO {
    ...
    fn addProposal(action: Actions) -> u32 {
        ...
        proposals::write(
            num_proposals::read(),
            Proposal { id: num_proposals::read(), yes_votes: 0, no_votes: 0, action: action }
        );
    ...
    fn vote(proposalId: u32, vote: bool) {
        assert(members::read(get_caller_address()), 'Caller not in DAO');
        assert(!voted::read((proposalId, get_caller_address())), 'Caller has voted');
    ...
    fn executeProposal(proposalId: u32) {
        ...
        assert(verify_proposal_approved(proposal_to_execute), 'Proposal has not been accepted');
